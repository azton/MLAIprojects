...
class UpConv(nn.Module):
    def __init__(self, in_channels, out_channels, filter_size=2, stride=1):
...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.Exp2 = ExpansionBlock(256+128, 128, 128)
        self.Exp3 = ExpansionBlock(128, 64, 64)

        ## final 1x1x1 convolution to get right number of outputs
        self.FinalConv = BasicConv(64, out_channels, filter_size=1)
    ...
    def forward(self, x):
        ...
        x = self.Exp1(x,catx3)
        x = self.upConv2(x)
        catx2 = self.crop(catx2)
        # print(x.size(), catx2.size())
        x = self.Exp2(x, catx2)
        x = self.upConv3(x)
        catx1= self.crop(catx1)
